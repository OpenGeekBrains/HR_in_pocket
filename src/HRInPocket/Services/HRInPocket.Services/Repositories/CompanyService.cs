...
namespace HRInPocket.Services.Repositories
{
    public class CompanyService : ICompanyService
    {
        ...
        public async Task<PageDTOs<CompanyDTO>> GetAllAsync(Filter filter = null)
        {
            ...
            {
                Entities = query.AsEnumerable().Select(_Mapper.Map<CompanyDTO>),
                TotalCount = count
            };
        }
        ...
        public async Task<CompanyDTO> GetByIdAsync(Guid id) => _Mapper.Map<CompanyDTO>(await _DataProvider.GetByIdAsync(id));

        /// <summary>
        /// Создать компанию
        /// </summary>
        /// <param name="company">Модель компании</param>
        public async Task<Guid> CreateAsync(CompanyDTO company) => await _DataProvider.CreateAsync(_Mapper.Map<Company>(company));

        /// <summary>
        /// Редактирвание информации о компании
        /// </summary>
        /// <param name="company">Модель компании</param>
        public async Task<bool> EditAsync(CompanyDTO company) => await _DataProvider.EditAsync(_Mapper.Map<Company>(company));
